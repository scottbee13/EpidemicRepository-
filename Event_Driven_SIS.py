...
def Event_Driven_SIS(G, τ, γ, initial_infecteds , tmax, seed_value = 100):
    ...
    times, S, I, N = [0], [len(list(G.nodes()))-initial_infecteds], [initial_infecteds], range(len(list(G.nodes())))
    ...
    while Priority_Queue.size != 0:
        ...
        if CurrentEvent[2] == 1.0:
            ...
            if G.node[CurrentEvent[0]]["Status"] == "Susceptible":
                ...
                if TempS + TempI != len(list(N)):
                    print("Unfortunately, S + I  does not equal N.")
                    
                if not (S[-1] == TempS and TempI == I[-1]):
                    S.append(len(Susceptible_Nodes))
                    I.append(len(Infected_Nodes))
            ...
            if G.node[CurrentEvent[0]]["Status"] == "Infected":
                G.node[CurrentEvent[0]]["Status"] = "Susceptible"
                Infected_Nodes.remove(int(CurrentEvent[0]))
                Susceptible_Nodes.append((int(CurrentEvent[0])))
...
times, S, I = Event_Driven_SIS(G, τ, γ, initial_infecteds , tmax, seed_value) 
